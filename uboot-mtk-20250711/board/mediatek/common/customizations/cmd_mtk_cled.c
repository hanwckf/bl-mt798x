...
static void cled_cyclic(struct cyclic_info *c)
{
	struct cled_info *info = container_of(c, struct cled_info, cyclic);
					...
					u32 pwm_ch_g, u32 pwm_ch_b)
{
	...
	cyclic_register(&info->cyclic, cled_cyclic, CLED_DEMO_DELAY * 1000,
			info->name);
...
static const struct mtk_led_demo mtk_led_demo_data[] = {
	...
	{
		.demo_name = "mt7987",
		.desc = "MT7987 LED DEMO",
		/*
		 * PIN13 (PWM0) : color-led (Red)
		 * PIN7  (PWM1) : color-led (Green)
		 * PIN12 (PWM2) : color-lde (Blus)
		 */
		.pins = {13, 7, 12},
		.pin_gpio_mode = {0, 0, 0},
		.pin_pwm_mode = {1, 3, 2},
...
U_BOOT_CMD(cled, 6, 0, do_cled,
	   ...
	   "valid demo_name: [mt7981, mt7988_1, mt7988_2, mt7987]\n"
	   "valid type: [gpio, pwm]\n"
