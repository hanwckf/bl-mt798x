...
static const struct toc_entry toc_entries[MAX_TOE_ENTRY] = {
	...
	{ .desc = "Platform owned Secure Partition content certificate",
	  ...
	  .name = "plat-sp-cert" },
	{ .desc = "FIP checksum data",
	  .uuid = UUID_FIP_CHKSUM,
	  .name = "fip-chksum" },
	{
		.desc = NULL,
		.uuid = UUID_NULL,
	       ...
	       size_t *data_len_out)
{
	...
	static const union fip_uuid uuid_chksum = UUID_FIP_CHKSUM;
	...
	list_for_each_entry(desc, &fip->images, list) {
		if (!memcmp(&desc->toc_e.uuid, &uuid_chksum,
			    sizeof(union fip_uuid))) {
			chksum.len = desc->toc_e.offset_address;
			chksum.crc = crc32_wd(0, data_out, chksum.len,
					      CHUNKSZ_CRC32);

			sha256_csum_wd(data_out, chksum.len, chksum.sha256sum,
				       CHUNKSZ_SHA256);

			chksum.crc = cpu_to_le32(chksum.crc);
			chksum.len = cpu_to_le32(chksum.len);

			memset(data_out + desc->toc_e.offset_address + sizeof(chksum),
			       0, desc->toc_e.size - sizeof(chksum));

			memcpy(data_out + desc->toc_e.offset_address, &chksum,
			       sizeof(chksum));
		} else {
...
int init_fip(const void *data, size_t size, struct fip *fip)
{
	...
	for (; (u8 *)(toc_entry + 1) <= end; toc_entry++) {
		...
		if (end - (u8 *)toc_entry < sizeof(struct fip_toc_entry)) {
			printf("%s: incomplete FIP toc entries\n", __func__);
			ret = -EINVAL;
			goto err;
		}
		...
		if (!memcmp(&toc_entry->uuid, &uuid_null,
			    sizeof(union fip_uuid))) {
			if (toc_entry->size) {
				printf("%s: Null toc entry has size != 0\n",
				       __func__);
				ret = -EINVAL;
				goto err;
			}

			if (toc_entry->offset_address > size) {
				printf("%s: Null toc entry offset is invalid\n",
				       __func__);
				ret = -EINVAL;
				goto err;
			}
		...
		if (!common_desc) {
			printf("%s: unknown image in FIP file\n", __func__);
			ret = -EINVAL;
			goto err;
		}
		/* Check input values */
		...
		if (value < toc_entry->offset_address ||
			...
			printf("%s: invalid data offset/size in FIP toc entry\n",
			       __func__);
			ret = -EOVERFLOW;
			goto err;
		}
		...
		if (!memcmp(&toc_entry->uuid, &uuid_chksum,
			    sizeof(union fip_uuid))) {
			if (toc_entry->size < sizeof(struct mtk_fip_checksum)) {
				printf("%s: FIP checksum data is invalid\n",
				       __func__);
				ret = -EINVAL;
				goto err;
			}

			memcpy(&chksum, data + toc_entry->offset_address,
			       sizeof(struct mtk_fip_checksum));

			chksum.len = le32_to_cpu(chksum.len);
			chksum.crc = le32_to_cpu(chksum.crc);

			if (chksum.len != toc_entry->offset_address) {
				printf("%s: FIP checksum data mismatch\n",
				       __func__);
				ret = -EBADMSG;
				goto err;
			}

			if (chksum.crc != crc32_wd(0, data, chksum.len,
						   CHUNKSZ_CRC32)) {
				printf("%s: FIP checksum CRC32 mismatch\n",
				       __func__);
				ret = -EBADMSG;
				goto err;
			}

			sha256_csum_wd(data, chksum.len, sha256sum,
				       CHUNKSZ_SHA256);

			if (memcmp(chksum.sha256sum, sha256sum,
				   SHA256_SUM_LEN)) {
				printf("%s: FIP checksum SHA256 mismatch\n",
				       __func__);
				ret = -EBADMSG;
				goto err;
			}
	...
	if (IS_ENABLED(CONFIG_MTK_DUAL_FIP) && !has_checksum) {
		printf("%s: FIP checksum data not exist\n", __func__);
		ret = -EBADMSG;
		goto err;
	}
