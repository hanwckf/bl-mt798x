			      ...
			      struct fit_hashes *hashes)
{
	...
	fdt_for_each_subnode(noffset, fit, image_noffset) {
		...
		if (!strcmp(algo, "sha256")) {
			if (fit_value_len != SHA256_SUM_LEN) {
				printf("Warning: hash length mismatch in %s\n",
				       name);
				continue;
			}

			memcpy(hashes->sha256, fit_value, SHA256_SUM_LEN);
			hashes->flags |= FIT_HASH_SHA256;
		}
	}
			      ...
			      size_t rootfs_size, struct fit_hashes *hashes)
{
	...
	if (fit_rootfs_size > rootfs_size) {
		printf("Incomplete rootfs (0x%x recorded in FIT, 0x%zx actual)\n",
		       fit_rootfs_size, rootfs_size);
		return false;
	}
 ...
 *
 * @description:
 * Verify image with rootfs. FIT raw image (for SPI-NOR, kernel + rootfs),
 * itb image and TAR archive (for UBI and SD/eMMC) are supported.
 *
 * @param data: image to be parsed
 * @param size: size of the image
		      ...
		      struct fit_hashes *rootfs_hashes)
{
	...
	if (ii->type == IMAGE_ITB) {
		...
		if (kernel_hashes)
			memset(kernel_hashes, 0, sizeof(*kernel_hashes));

		if (rootfs_hashes)
			memset(rootfs_hashes, 0, sizeof(*rootfs_hashes));
...
/**
 * read_verify_itb_image() - Read and verify itb image from flash
 *
 * @description:
 * Read itb image from flash and then verify.
 *
 * @param rpriv: priv structure for flash read operation
 * @param ptr: Pointer to memory to where itb image will be read
 * @param addr: Address in flash from where itb image will be read
 * @param max_size: Maximum size allowed for itb image
 * @param actual_size: (optional) on return stores the actual itb image size
 * @return true if integrity verification passed
 */
			   ...
			   size_t max_size, size_t *actual_size)
{
	...
	probe_size = sizeof(struct fdt_header);
	...
	if (genimg_get_format(ptr) != IMAGE_FORMAT_FIT) {
		printf("Error: Not FIT image\n");
		return false;
	}

	fit_size = fit_get_size(ptr);
	if (max_size && fit_size > max_size) {
		printf("Error: incomplete FIT image found\n");
		return false;
	}

	ret = rpriv->read(rpriv, ptr, addr, fit_size);
	if (ret) {
		printf("Error: read failure at offset 0x%llx\n", addr);
		return false;
	}
	...
	ret = rpriv->read(rpriv, ptr, addr, image_size);
	if (ret) {
		printf("Error: read failure at offset 0x%llx\n", addr);
		return false;
	}

	if (fit_check_format(ptr, image_size)) {
		printf("Wrong FIT image format\n");
		return false;
	}

	if (!fit_all_image_verify(ptr)) {
		printf("FIT image integrity checking failed\n");
		return false;
	}
		    ...
		    const struct fit_hashes *hash2)
{
	...
	if (common_flags & FIT_HASH_SHA256) {
		if (memcmp(hash1->sha256, hash2->sha256, SHA256_SUM_LEN)) {
