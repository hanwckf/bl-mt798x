...
static uintptr_t mt_gpio_find_reg_addr(uint32_t pin)
{
	...
	gpio_info = mt7988_pin_infos[pin];
...
static void mt_gpio_set_pull_chip(uint32_t pin, int enable, int select)
{
	...
	pin_info_cnt = sizeof(mt7988_pin_infos) / sizeof(struct mt_pin_info);

	for (idx = 0; idx < pin_info_cnt; ++idx) {
		if (mt7988_pin_infos[idx].id == pin) {
			gpio_info = mt7988_pin_infos[idx];
			break;
		}
	}
...
#ifdef ENABLE_JTAG
	/* ENABLE JTAG */
	mt_set_pinmux_mode(58, GPIO_MODE_01);
	mt_set_pinmux_mode(59, GPIO_MODE_01);
	mt_set_pinmux_mode(60, GPIO_MODE_01);
	mt_set_pinmux_mode(61, GPIO_MODE_01);
	mt_set_pinmux_mode(62, GPIO_MODE_01);
#else
	/* DISABLE JTAG */
	mt_set_pinmux_mode(58, GPIO_MODE_00);
	mt_set_pinmux_mode(59, GPIO_MODE_00);
	mt_set_pinmux_mode(60, GPIO_MODE_00);
	mt_set_pinmux_mode(61, GPIO_MODE_00);
	mt_set_pinmux_mode(62, GPIO_MODE_00);
#endif
	/* PMIC */
	mt_set_pinmux_mode(15, GPIO_MODE_00);
	mt_set_pinmux_mode(16, GPIO_MODE_00);
}
